...
extends Behaviour
## Carry a spoil pellet from the digging face up through the nest entrance
## and drop it on the spoil heap (NestType.spoil_position(), within
## `spoil_spread`, chosen once on the surface so the heap grows as a mound),
## then -> `on_done`. Heavier pellets slow the ant like any load.
##
...
func tick(sim: Simulation, i: int, dt: float) -> String:
	...
	if sim.layer[i] != 0:
		Travel.go(sim, i, 0, nest.spoil_position(), -1, move_speed, dt)
		return ""
	if sim.scratch_f0[i] == 0.0:
		var spread: float = p.get("spoil_spread", 22.0)
		sim.target[i] = nest.spoil_position() + Vector2.from_angle(sim.rng.randf() * TAU) * spread * sqrt(sim.rng.randf())
		sim.scratch_f0[i] = 1.0
	if Travel.go(sim, i, 0, sim.target[i], -1, move_speed, dt, 4.0):
		var dropped := sim.drop_item(i)
		nest.receive_spoil(sim, dropped)
