...
func tick(sim: Simulation, i: int, dt: float) -> String:
	...
	var turn := 0.0
	if nest.has_entrance() and at.distance_squared_to(nest.nearest_entrance(at)) < nest.sense_radius * nest.sense_radius:
		turn = Steering.turn_toward(at, sim.heading[i], nest.nearest_entrance(at))
	else:
		...
		if nest.has_entrance() and sim.since_obstacle[i] >= colony.obstacle_memory:
			turn = Steering.home_turn(at, sim.heading[i], nest.nearest_entrance(at), turn,
