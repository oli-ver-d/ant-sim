...
static func create(sim: Simulation, spec: Dictionary) -> Item:
	...
	if kind == "pebble":
		...
		item.shape = _twig_image(length, rng)
...
## A slightly bent stick along +x with a side sprig, bark shading and ends.
static func _twig_image(length: float, rng: RandomNumberGenerator) -> Image:
	var w := int(ceil(length * RES)) + 4
	var h := int(ceil(14.0 * RES))
	var img := Image.create_empty(w, h, false, Image.FORMAT_RGBA8)
	var base := Color(0.44, 0.32, 0.2).lerp(Color(0.55, 0.42, 0.28), rng.randf())
	var bend := rng.randf_range(-2.0, 2.0) * RES
	var thick := rng.randf_range(2.2, 3.0) * RES
	...
	for x in w:
		...
		for y in h:
			...
			if t > sprig_at and t < sprig_at + 0.22:
				...
				sprig = absf(y + 0.5 - (centre + sprig_dir * st * 5.0 * RES)) / (thick * 0.45)
			var m := minf(d, sprig)
			if m < 1.0:
